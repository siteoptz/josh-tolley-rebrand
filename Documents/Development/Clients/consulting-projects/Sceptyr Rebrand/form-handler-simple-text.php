<?php
// Simplified version - use text format for all fields
...
// Monday.com integration - simplified format
if (!empty($first_name) && !empty($last_name)) {
    try {
        ...
        // Use simple text format for ALL fields
        $column_values = [
            'text' => $first_name,
            'text_1' => $last_name,
            'email_mm06cmw7' => $email,                    // Just the email string
            'phone_mm06bhs1' => $phone,                    // Just the phone string  
            'text_mm02dymw' => $net_worth,                 // Net worth as text
            'status_1_mm06w6j3' => $accredited,           // Just the status string
            'dropdown_mm02jnrm' => $interest,             // Just the interest string
            'text17_mm02fh7q' => $message                 // Message as text
        ];
        ...
        if ($response && $http_code === 200) {
            ...
            if (isset($result['data']['create_item']['id'])) {
                ...
                $results['monday_result']['column_values_sent'] = $column_values;
            } else {
                $results['monday_result']['error'] = $response;
            }
        } else {
            $results['monday_result']['error'] = "HTTP $http_code: $response";
            $results['monday_result']['column_values_attempted'] = $column_values;
        }
...
if (function_exists('mail')) {
    try {
        ...
        if (!$results['email_result']['success']) {
            $results['email_result']['error'] = 'mail() returned false';
        }
